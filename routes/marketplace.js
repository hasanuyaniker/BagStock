      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (needsNameUpdate && productName) {
            await pool.query(
              `UPDATE marketplace_order_items
               SET product_name      = $1,
                   ...
                   product_id        = COALESCE($3, product_id),
                   barcode           = CASE WHEN $4 != '' AND (barcode IS NULL OR barcode ~ '^[0-9]{8,}$') THEN $4 ELSE barcode END,
                   commission_amount = CASE WHEN $6::numeric > 0 THEN $6::numeric ELSE commission_amount::numeric END,
                   commission_rate   = CASE WHEN $7::numeric IS NOT NULL THEN $7::numeric ELSE commission_rate::numeric END
               WHERE marketplace_order_id = $5
                 AND (product_name IS NULL OR product_name = '' OR product_name ~ '^[0-9]{8,}$')`,
            ...
            await pool.query(
              `UPDATE marketplace_order_items
               SET commission_amount = CASE WHEN $1::numeric > 0 THEN $1::numeric ELSE commission_amount::numeric END,
                   commission_rate   = CASE WHEN $2::numeric IS NOT NULL THEN $2::numeric ELSE commission_rate::numeric END
          ...
          await pool.query(
            `UPDATE marketplace_orders
             SET customer_name     = CASE WHEN $1 != '' THEN $1 ELSE customer_name END,
                 ...
                 commission_amount = CASE WHEN $3::numeric > 0 THEN $3::numeric ELSE commission_amount::numeric END,
                 commission_rate   = CASE WHEN $4::numeric IS NOT NULL THEN $4::numeric ELSE commission_rate::numeric END,
                 updated_at        = NOW()
             WHERE id = $5`,
